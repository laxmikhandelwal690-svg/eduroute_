  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 md:p-8 bg-slate-100/60">
      <header className="mb-10 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">Welcome back, {MOCK_USER.name.split(' ')[0]}! 👋</h1>
          <p className="mt-2 text-slate-500 font-medium">You've completed 45% of your current path. Keep it up!</p>
        </div>
        <div className="flex items-center gap-4 rounded-3xl bg-white p-6 border border-slate-200 shadow-sm">
          <div className="rounded-2xl bg-slate-100 p-3 text-slate-700">
            <ShieldCheck className="h-6 w-6" />
          </div>
          <div>
            <div className="text-sm font-bold text-slate-900">Verify College ID</div>
            <p className="text-xs text-slate-500 font-semibold">Unlock 50% discount on all certifications</p>
          </div>
          <button className="ml-4 rounded-xl bg-slate-900 px-4 py-2 text-xs font-semibold text-white hover:bg-slate-700 transition-colors">
            Verify
          </button>
        </div>
      </header>
      ...
      <section className="mb-12">
        <div className="mb-8 flex items-center justify-between">
          ...
          <button className="text-sm font-semibold text-slate-600 hover:text-slate-900">Explore</button>
        </div>
        ...
        <div className="rounded-[32px] bg-slate-900 p-10 text-white shadow-xl">
          <div className="flex flex-col items-center justify-between gap-12 md:flex-row">
            <div className="space-y-6">
              <h2 className="text-3xl font-bold">Weekly Goal Progress</h2>
              <div className="flex gap-6">
                <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-white/20 backdrop-blur-md">
                  <Trophy className="h-8 w-8" />
                </div>
                <div>
                  <div className="text-sm font-semibold uppercase tracking-widest opacity-80">Points earned</div>
                  <div className="text-3xl font-bold">340 / 500</div>
                </div>
              </div>
            ...
            <button className="flex items-center gap-3 rounded-[20px] bg-white px-10 py-5 font-semibold text-slate-900 shadow-lg transition-transform hover:scale-105 active:scale-95">
...
const ContinueLearningCard = ({ course }: { course: Course }) => (
  <motion.div 
    whileHover={{ y: -4 }}
    className="group flex flex-col gap-6 overflow-hidden rounded-[28px] border border-slate-200/70 bg-white p-6 transition-all hover:shadow-lg sm:flex-row"
  >
    ...
    <div className="flex flex-1 flex-col justify-center py-2">
      ...
      <h3 className="mt-2 text-xl font-semibold text-slate-900 group-hover:text-slate-700 transition-colors leading-tight">{course.title}</h3>
      <div className="mt-6 h-2 w-full rounded-full bg-slate-100">
        <div className="h-full w-[40%] rounded-full bg-slate-900"></div>
      </div>
    </div>
  </motion.div>
);
...
const CourseCard = ({ course }: { course: Course }) => (
  <motion.div 
    whileHover={{ y: -8 }}
    className="group overflow-hidden rounded-[28px] border border-slate-200/70 bg-white transition-all hover:shadow-lg"
  >
    ...
    <div className="p-8">
      ...
      <h3 className="mt-4 line-clamp-2 text-xl font-semibold text-slate-900 group-hover:text-slate-700 transition-colors leading-snug h-14">
        {course.title}
      </h3>
