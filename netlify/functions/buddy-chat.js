...
exports.handler = async (event) => {
  ...
  try {
    ...
    try {
      ...
      dbEnabled = false;
      console.warn('buddy-chat DB disabled, serving stateless response', dbError.message);
    }

    const recentMessages = dbEnabled
      ? profile.chatHistory.slice(-12).map((entry) => ({ role: entry.role === 'assistant' ? 'assistant' : 'user', content: entry.text }))
      : [{ role: 'user', content: message }];

    const aiReply = await generateBuddyReply({ messages: recentMessages, language });

    const basePoints = dbEnabled ? profile.points || 0 : 0;
    const pointsEarned = 5;
    const newPoints = basePoints + pointsEarned;
    ...
    return json(200, {
      ok: true,
      reply: aiReply,
      mode: dbEnabled ? 'persistent' : 'stateless',
      gamification: {
        points: newPoints,
        level: newLevel,
        pointsEarned,
